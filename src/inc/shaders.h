...
inline static MEL_FN MEL_str2shader(const char * vert_path, const char * frag_path){
	...
	if (success == GL_FALSE){
		glGetShaderInfoLog(vertex_shader, 1024, NULL, errnoo);
		MEL_log(LOG_WARNING, "Failed compiling vertex shader: [%s]", errnoo);
		glDeleteShader(vertex_shader);
		return EX_F;
	}else{
		MEL_log(LOG_SUCCESS, "Successfully compiled vertex shader");
	}

    /* Create vertex shader object and its reference */
    GLuint fragment_shader = glCreateShader(GL_FRAGMENT_SHADER);
    /* Attach vertex shader source to the vertex shader object */
    glShaderSource(fragment_shader, 1, (const GLchar * const *)&frag_path, NULL);
    /* Compile the vertex shader */
    glCompileShader(fragment_shader);
	...
	if (success == GL_FALSE){
		glGetShaderInfoLog(fragment_shader, 1024, NULL, errnoo);
		MEL_log(LOG_WARNING, "Failed compiling fragment shader: [%s]", errnoo);
		glDeleteShader(vertex_shader);
		glDeleteShader(fragment_shader);
		return EX_F;
	}else{
		MEL_log(LOG_SUCCESS, "Successfully compiled fragment shader");
	}
