...
MEL_Texture MEL_load_tex(MEL_Renderer2D * Renderer, GLchar * path,
  ...
  if (data) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, channels, img.rect.size[0],
                 img.rect.size[1], 0, channels, GL_UNSIGNED_BYTE, data);
...
void MEL_draw_tex(mel_t ctx, MEL_Renderer2D * Renderer, MEL_Texture * Img,
  ...
  if (is_visible(ctx, Img->rect, Camera)) {
    ...
    glm_rotate_at(Img->rect.mvp, (vec3) {
                  (float) (Img->rect.pos[0] + Img->rect.size[0] / 2.0f),
                  (float) (Img->rect.pos[1] + Img->rect.size[1] / 2.0f),
                  0.0f}, glm_rad(Img->rect.rotation), (vec3) {
                  0.0f, 0.0f, 1.0f});
