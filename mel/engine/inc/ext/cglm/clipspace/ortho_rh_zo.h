...
CGLM_INLINE void glm_ortho_aabb_rh_zo(vec3 box[2], mat4 dest) {
  glm_ortho_rh_zo(box[0][0], box[1][0],
                  box[0][1], box[1][1], -box[1][2], -box[0][2], dest);
}
 ...
 * @param[out] dest    result matrix
 */
CGLM_INLINE void glm_ortho_aabb_p_rh_zo(vec3 box[2], float padding, mat4 dest) {
 ...
 * @param[out] dest   result matrix
 */
CGLM_INLINE void glm_ortho_default_rh_zo(float aspect, mat4 dest) {
  if (aspect >= 1.0f) {
...
CGLM_INLINE
  ...
  if (aspect >= 1.0f) {
    glm_ortho_rh_zo(-size * aspect,
                    size * aspect,
                    -size, size, -size - 100.0f, size + 100.0f, dest);
    return;
  }
  ...
  glm_ortho_rh_zo(-size,
                  size,
                  -size / aspect,
                  size / aspect, -size - 100.0f, size + 100.0f, dest);
}
