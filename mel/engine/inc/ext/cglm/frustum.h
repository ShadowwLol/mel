 ...
 */
CGLM_INLINE void glm_frustum_center(vec4 corners[8], vec4 dest) {
  vec4 center;
 ...
 */
CGLM_INLINE void glm_frustum_box(vec4 corners[8], mat4 m, vec3 box[2]) {
  vec4 v;
  vec3 min, max;
  int i;
...
glm_frustum_corners_at(vec4 corners[8],
                       float splitDist, float farDist, vec4 planeCorners[4]) {
  vec4 corner;
  float dist, sc;
