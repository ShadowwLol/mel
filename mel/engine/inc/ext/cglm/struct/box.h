 ...
 */
CGLM_INLINE void glms_aabb_transform(vec3s box[2], mat4s m, vec3s dest[2]) {
  vec3 rawBox[2];
  vec3 rawDest[2];
 ...
 */
CGLM_INLINE void glms_aabb_merge(vec3s box1[2], vec3s box2[2], vec3s dest[2]) {
  vec3 rawBox1[2];
  vec3 rawBox2[2];
  vec3 rawDest[2];
 ...
 */
CGLM_INLINE void glms_aabb_crop(vec3s box[2], vec3s cropBox[2], vec3s dest[2]) {
  vec3 rawBox[2];
  vec3 rawCropBox[2];
  vec3 rawDest[2];
...
glms_aabb_crop_until(vec3s box[2],
                     vec3s cropBox[2], vec3s clampBox[2], vec3s dest[2]) {
 ...
 */
CGLM_INLINE bool glms_aabb_frustum(vec3s box[2], vec4s planes[6]) {
  vec3 rawBox[2];
  vec4 rawPlanes[6];
 ...
 */
CGLM_INLINE void glms_aabb_invalidate(vec3s box[2]) {
 ...
 * @param[in]  box bounding box
 */
CGLM_INLINE bool glms_aabb_isvalid(vec3s box[2]) {
  vec3 rawBox[2];
  glms_vec3_unpack(rawBox, box, 2);
 ...
 * @param[in]  box bounding box
 */
CGLM_INLINE float glms_aabb_size(vec3s box[2]) {
 ...
 * @param[in]  box bounding box
 */
CGLM_INLINE float glms_aabb_radius(vec3s box[2]) {
 ...
 */
CGLM_INLINE bool glms_aabb_aabb(vec3s box[2], vec3s other[2]) {
  vec3 rawBox[2];
  vec3 rawOther[2];
 ...
 */
CGLM_INLINE bool glms_aabb_sphere(vec3s box[2], vec4s s) {
  vec3 rawBox[2];
 ...
 */
CGLM_INLINE bool glms_aabb_point(vec3s box[2], vec3s point) {
  vec3 rawBox[2];
 ...
 * @param[in]   other  other bounding box
 */
CGLM_INLINE bool glms_aabb_contains(vec3s box[2], vec3s other[2]) {
  vec3 rawBox[2];
  vec3 rawOther[2];
