 ...
 */
CGLM_INLINE float glm_sphere_radii(vec4 s) {
  return s[3];
}
 ...
 */
CGLM_INLINE void glm_sphere_transform(vec4 s, mat4 m, vec4 dest) {
 ...
 */
CGLM_INLINE void glm_sphere_merge(vec4 s1, vec4 s2, vec4 dest) {
  float dist, radii;
 ...
 */
CGLM_INLINE bool glm_sphere_sphere(vec4 s1, vec4 s2) {
 ...
 */
CGLM_INLINE bool glm_sphere_point(vec4 s, vec3 point) {
  float rr;
  rr = s[3] * s[3];
